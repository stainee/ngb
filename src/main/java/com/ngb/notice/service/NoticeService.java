...
public class NoticeService {
	...
	public NoticePageData selectNoticeList(int reqPage, String notice_title, String searchInput) {
		Connection conn = JDBCTemplate.getConnection();
		//한페이지 게시물 10
		int numPerPage = 10;
		//끝은 한페이지 게시물*현재 몇페이지인지
		...
		int end = numPerPage*reqPage;
		int start = end - numPerPage + 1;
		ArrayList<Notice> list = dao.selectNoticeList(conn,notice_title,searchInput,start,end);
		
		//페이징처리
		//전체페이지 수 계산 -> 전체 게시물 수를 조회
		int totalCount = dao.selectNoticeCount(conn,notice_title,searchInput);
		//전체 페이지
		int totalPage = 0;
		...
		if(pageNo != 1) {
			pageNavi += "<li>";
			pageNavi += "<a class='page-item' href='/noticeSearch.do?reqPage="+(pageNo-1)+"&searchTag="+notice_title+"&searchInput="+searchInput+"'>";
			pageNavi += "<span class='material-icons'>chevron_left</span>";
		...
		for(int i=0;i<pageNaviSize;i++) {
			if(pageNo == reqPage) {
				pageNavi += "<li>";
				pageNavi += "<a class='page-item active-page' href='/noticeSearch.do?reqPage="+pageNo+"&searchTag="+notice_title+"&searchInput="+searchInput+"'>";
				pageNavi += pageNo;
				pageNavi += "</a></li>";
			}else {
				pageNavi += "<li>";
				pageNavi += "<a class='page-item' href='/noticeSearch.do?reqPage="+pageNo+"&searchTag="+notice_title+"&searchInput="+searchInput+"'>";
				pageNavi += pageNo;
				pageNavi += "</a></li>";
			}
			pageNo++;
