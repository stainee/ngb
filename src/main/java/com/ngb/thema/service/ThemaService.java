...
public class ThemaService {
	...
	public ArrayList<ThemaTimeManage> selectAllThemaList() {
		Connection conn = JDBCTemplate.getConnection();
	...
	public int addThemaTime(ThemaTime tt) {
		...
		int result = dao.deleteThamaTime(conn, tt.getThemaCode());
		System.out.println("지우는 쿼리 테스트"+result);
		if (result >= 0) {
			result = dao.addThemaTime(conn, tt);
			if (result == 10) {
				JDBCTemplate.commit(conn);
			} else {
				JDBCTemplate.rollback(conn);
